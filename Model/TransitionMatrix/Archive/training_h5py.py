...
class Word2Vec:
    ...
    def __init__(self, ifolder, ofolder, 
        ...
        posPairFile = h5py.File(self.outfolder+"PostivePairFile.hdf5","w")
        self.positive_pairs = posPairFile.create_dataset("posPairs", (self.pair_count,2), dtype=int)
        self.posReadFile = h5py.File(self.outfolder+"PostivePairFile.hdf5","r")
        ...
        negPairFile = h5py.File(self.outfolder+"NegativePairFile.hdf5","w")
        self.negative_pairs = negPairFile.create_dataset("negPairs", (self.pair_count,5), dtype=int)
        self.negReadFile = h5py.File(self.outfolder+"NegativePairFile.hdf5","r")
    ...
    def get_batch_pairs(self, batch_count):
        ...
        return self.posReadFile['posPairs'][(batch_count)*self.batch_size:(batch_count+1)*self.batch_size]
        #return self.positive_pairs[(batch_count)*self.batch_size:(batch_count+1)*self.batch_size]
        
        
    def get_neg_v(self, batch_count):
        
        return self.negReadFile['negPairs'][(batch_count)*self.batch_size:(batch_count+1)*self.batch_size]
