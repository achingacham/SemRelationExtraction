...
if __name__ == '__main__':
    ...
    ifolder = sys.argv[1]
    ofolder = sys.argv[2]
    print("\n Enter validationFile Tag EpochFile BatchSize #Epochs and PreTrainedEmb\n") 
    ...
    with open("Modelparams_Log","a") as paramFile:
        ...
        paramFile.write("\n L2_factor & Lr : "+str(l2_factor)+" & "+str(lr)+", for files "+ str(validationfile)+ " and "+str( epochFile) +" with batchSize "+ str(batchSize)+" for "+str(epochTrain)+" epochs")

    
    results = [[None]] * 3
    
    outfolder = ofolder+ifolder.rsplit('/',2)[1]+'/'+tag+'/'+epochFile+'/'
