...
class modelData:
    ...
    def __init__(self, blessFile):
        ...
        with open(blessFile) as inputFile:
                ...
                for line in inputFile:
                    ...
                    words = line.split(',')
                    keys = [i.strip(" ['") for i in words]
                    
                    try:
                        pairKey = "['"+keys[0]+"','"+keys[1]+"']"
                        self.dictTest.append(pairKey)
                        anotherKey = keys[0]+':::'+keys[1]
                        self.dictTest.append(anotherKey)
                        content.append(line.strip('\n'))
                        g
                    except:
                        pass
                print("\n Length of dictTest ",len(self.dictTest), self.dictTest[:2])
    ...
    def create_dictRelVectors(self,embeddingFile):
        ...
        print("\n Length of vector dictionary ", count)
        inputFile.close()
        return input_dim
    ...
    def make_batch_input_vector(self,batch_target,batch_relata):
        ...
        for (target,relata) in zip(batch_target,batch_relata):
            ...
            key = target+':::'+relata
            if key in self.dictRelVectors:
                
                relation_vector = [float(value) for value in self.dictRelVectors[key]] 
                batch_relation_vector.append(relation_vector)
            else:
                
                anotherKey = "['"+target+"','"+relata+"']"
                if anotherKey in self.dictRelVectors:
                    
                    relation_vector = [float(value) for value in self.dictRelVectors[anotherKey]]
                    batch_relation_vector.append(relation_vector)
    ...
    def make_input_vector(self,target,relata):
        ...
        anotherkey = pairKey = "['"+target+"','"+relata+"']"
        if anotherkey in self.dictRelVectors:
            relation_vector = [float(value) for value in self.dictRelVectors[anotherkey]] 
            tensor = torch.cuda.FloatTensor(relation_vector)
...
def train(labelCount, logfile):
    ...
    plot_results(epochCostDev, "Dev data",outfolder+"DevPlot_justRelVectors.png") 
...
if __name__ == '__main__':
    ...
    validatedBlessSet = ifolder+"CombinedSet.txt"
    validatedRelationEmbedding = ifolder+"EMB_All.txt"
    ...
    plot_results(train_cost, "Epochs",outfolder+"TrainPlot_justRelVectors.png")
